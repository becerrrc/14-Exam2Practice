...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        s = ''
        sumstring = self.contents + additional_contents
        if len(self.contents) + len(additional_contents) <= self.volume:
            ...
            for k in range(0, self.volume):
                self.contents = self.contents + sumstring[k]
            for k in range(self.volume, len(sumstring)):
                s = s + sumstring[k]
            return s
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        self.reset_sequence = self.reset_sequence + [self.contents]
